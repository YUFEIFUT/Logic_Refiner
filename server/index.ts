...
app.get("/api/refine", async (req, res) => {
  ...
  try {
    ...
    const architectOutput = await llm.generate(architectPrompt, architectSystem, { thinking: true });
    ...
    for (let c = 1; c <= cycles; c++) {
      ...
      const redTeamOutput = await llm.generate(redTeamPrompt, redTeamSystem, { thinking: true });
      ...
      const synthesizerOutput = await llm.generate(synthesizerPrompt, synthesizerSystem, { thinking: true });
    ...
    const boundaryOutput = await llm.generate(boundaryPrompt, boundarySystem, { thinking: true });
    ...
    const finalLogic = await llm.generate(crystallizationPrompt, crystallizationSystem, { thinking: true });
    ...
    const explanation = await llm.generate(explainerPrompt, explainerSystem, { thinking: true });
